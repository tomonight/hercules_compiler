...
func (p *parser) fileOrNil() *File {
	...
	for p.tok != _EOF {
		...
		case _Const:
			p.next()
			addConstDecl(p.constDecl())
		case _Func:
			p.next()
			if d := p.funcDeclOrNil(); d != nil {
				f.DeclList = append(f.DeclList, d)
				d.Package = p.file.Filename()
				err := addFunc(d)
				if err != nil {
...
func (p *parser) funcDeclOrNil() *FuncDecl {
	...
	f.Name = p.name()
	f.Type = p.funcType()
	if p.tok == _Lbrace || (p.got(_Semi) && p.got(_Lbrace)) {
		f.Body = p.funcBody()
	}
	f.Pragma = p.pragma
...
func (p *parser) typeOrNil() Expr {
	...
	case _Lbrack:
		...
		c.Type = t
		list := p.exprList()
		if _, ok := list.(*ListExpr); !ok {
			c.ElemList = append(c.ElemList, list)
		} else {
			listExpr := list.(*ListExpr)
			for _, v := range listExpr.ElemList {
				c.ElemList = append(c.ElemList, v)
			}
		}
